...
export async function createUser(username, passwordHash) {
  ...
  const userCreated = await db.collection(COLLECTIONS.USERS).insertOne({
    userID: newUserID,
    username: username,
    password_hash: passwordHash,
    createdAt: new Date().toISOString(),
    profile_banner_phrase:
...
export async function getUser(userID) {
  return await db
    .collection(COLLECTIONS.USERS)
    .findOne({ userID: userID }, { projection: { password_hash: 0 } });
}
...
export async function updateUser(userID, updatedFields) {
  const updateResult = await db
    .collection(COLLECTIONS.USERS)
    .updateOne({ userID: userID }, { $set: updatedFields });
  if (
    updateResult &&
    ...
    throw new Error(
      "User not found or no changes made. Result: ",
      updateResult,
    );
  }
}
